...
def normalize_split_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    required_columns = {"snippet", "snippet_paths", "fallacy"}
    ...
    if "text" in df.columns:
        column_map["text"] = "snippet"
    if "audio" in df.columns:
        column_map["audio"] = "snippet_paths"
    if "label" in df.columns:
        column_map["label"] = "fallacy"

    normalized_df = df.rename(columns=column_map)

    missing_columns = required_columns.difference(normalized_df.columns)
    if missing_columns:
        raise ValueError(
            "Split dataframe is missing required columns after normalization: "
            f"{sorted(missing_columns)}"
        )

    normalized_df["snippet_paths"] = normalized_df["snippet_paths"].apply(
        lambda paths: paths
...
def main():
    ...
    parser.add_argument("audio_config_name", type=str, nargs='?', help="Name of the audio configuration to use", default="config_8layers_bow_False_random_80")
    ...
    train_data1 = normalize_split_dataframe(train_data1)
    val_data1 = normalize_split_dataframe(val_data1)
    test_data1 = normalize_split_dataframe(test_data1)
    ...
    output_dir = "../train_val_test_data_llama"
