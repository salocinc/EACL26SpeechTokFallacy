...
def main(seed=42, learning_rate=1e-3):
    ...
    save_path = Path(__file__).parent.parent.parent.resolve().joinpath('results', 'mmused-fallacy', 'afc', 'baseline_roberta',
    ...
    # Load data directly from pickle files like in baseline_bow_only.py
    ...
    # Use only one seed instead of iterating through multiple seeds
    processor = UnimodalProcessor()
    processor.fit(data.train)

    train_data_processed = processor(data.train)
    val_data_processed = processor(data.val)
    test_data_processed = processor(data.test)
    ...
    train_dataloader = DataLoader(train_data_processed,
                                  batch_size=config.batch_size,
                                  shuffle=True,
                                  collate_fn=unimodal_collator)
    val_dataloader = DataLoader(val_data_processed,
                                batch_size=config.batch_size,
                                shuffle=False,
                                collate_fn=unimodal_collator)
    test_dataloader = DataLoader(test_data_processed,
                                 batch_size=config.batch_size,
                                 shuffle=False,
                                 collate_fn=unimodal_collator)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Baseline RoBERTa model for fallacy classification")
    parser.add_argument(
        "--seed", 
        type=int, 
        default=42, 
    ...
    print(f"Starting baseline RoBERTa training with:")
