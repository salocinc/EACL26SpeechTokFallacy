...
from mamkit.configs.text import BowConfig
from mamkit.data.collators import BoWTextCollator, UnimodalCollator
from mamkit.data.datasets import MMUSEDFallacy, InputMode
from mamkit.data.processing import UnimodalProcessor, BagOfWordsProcessor
from mamkit.models.text import BoW
...
def main(seed=42, learning_rate=1e-3):
    ...
    save_path = Path(__file__).parent.parent.parent.resolve().joinpath('results', 'mmused-fallacy', 'afc', 'baseline_bow_only',
    ...
    config = BowConfig.from_config(key=ConfigKey(dataset='mmused-fallacy',
    ...
    processor = UnimodalProcessor(features_processor=BagOfWordsProcessor(max_features=10000),
                                )
    ...
    collator = UnimodalCollator(
        features_collator=BoWTextCollator(),
    ...
    model = BoW(head=lambda: th.nn.Sequential(
                                th.nn.Linear(len(processor.features_processor.vocab), 6)
                                ))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Baseline BoW model for fallacy classification")
    parser.add_argument(
        "--seed", 
        type=int, 
        default=42, 
    ...
    print(f"Starting baseline BoW training with:")
