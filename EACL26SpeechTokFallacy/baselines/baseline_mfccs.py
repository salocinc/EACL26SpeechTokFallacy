...
# -- Agrega el directorio raíz del repo a sys.path
...
from mamkit.configs.audio import MFCCsConfig
...
from mamkit.models.audio import MFCCs
...
def main(seed=42, learning_rate=1e-3):
    ...
    save_path = Path(__file__).parent.parent.parent.resolve().joinpath('results', 'mmused-fallacy', 'afc', 'baseline_mfccs',
    ...
    config = MFCCsConfig.from_config(key=ConfigKey(dataset='mmused-fallacy',
    ...
    model = MFCCs(
        head=lambda: th.nn.Sequential(
            th.nn.Linear(config.audio_embedding_dim, 6)
        )
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Baseline MFCCs model for fallacy classification")
    parser.add_argument(
        "--seed", 
        type=int, 
        default=42, 
    ...
    print(f"Starting baseline MFCCs training with:")
