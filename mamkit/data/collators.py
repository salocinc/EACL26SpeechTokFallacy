...
class BoWTextCollator(CollatorComponent):
    """
    Bag of Words (BoW) text collator that directly processes precomputed BoW vectors.
    """

    def __init__(self):
        """
        Initializes the BoWTextCollator.
        No tokenizer or vocabulary is required since BoW vectors are precomputed.
        """
        pass
    ...
    def __call__(
        ...
        texts = [th.tensor(text, dtype=th.float32) if text is not None
             else th.tensor([0], dtype=th.int32) for text in inputs]
        texts = pad_sequence(texts, padding_value=0, batch_first=True)
        context = [th.tensor(text, dtype=th.float32) if text is not None
             else th.tensor([0], dtype=th.int32) for text in context]
