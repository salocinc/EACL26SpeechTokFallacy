...
class BoWMFCCs(TextAudioModel):
    def __init__(self, head):
        super().__init__()
        self.head = head()

    def forward(self, inputs):
        # if someone passed (inputs_dict, labels), grab the first element
        if isinstance(inputs, (tuple, list)):
            inputs = inputs[0]
            
        # Text input
        text_features = inputs['text_inputs']

        # Process audio input
        audio_features = inputs['audio_inputs']
        audio_features_pooled = th.mean(audio_features, dim=1)

        # Concatenate and pass through the head
        concat_emb = th.concat((text_features, audio_features_pooled), dim=-1)
        logits = self.head(concat_emb)
        return logits
